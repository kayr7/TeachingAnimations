...
class DiracDistribution(Scene):
    def construct(self):
        ...
        def gaussian(x, mu=0, epsilon=1):
            value = (1.0 / np.sqrt(epsilon * 2.0 * np.pi)) * np.exp(-(x - mu)**2 / (2 * epsilon))
        ...
        graph = axes.plot(lambda x: gaussian(x, epsilon=epsilon), color=RED)
        ...
        for _ in range(25):
            ...
            new_graph = axes.plot(lambda x: gaussian(x, epsilon=epsilon), color=RED)
